...
namespace SocketModule
{
	public class PacketProtocol
	{
		...
		public void DataReceived(byte[] data)
		{
			...
			Console.WriteLine ("receive data: {0}", data.Length);
		...
		private void ReadCompleted(int count)
		{
			...
			if (this.dataBuffer == null)
			{
				...
				else
				{
					...
					int length = (int)CodedInputStream.CreateInstance(this.lengthBuffer).ReadRawVarint32();
					Console.WriteLine("receive data length: {0}", length);
			...
			else
			{
				...
				else
				{
					...
					if (this.MessageArrived != null) {
						...
						Console.WriteLine("length: {0}", this.dataBuffer.Length);
						Response response = Response.ParseFrom(this.dataBuffer);
						this.channelRead(response);
					}
		...
		public void channelRead(Response response)
		{
			Console.WriteLine ("receive: {0}", response.ToString());
		}
	}
}
